 ...
 *
 * cycle function:
 * https://doc.rust-lang.org/std/iter/trait.Iterator.html#method.cycle
 *
...
public:
    ...
    static_assert(
        std::is_copy_assignable<reverse_decay_t<Self>>::value,
        "Cycle can only take in copy assignable types. "
        "Operations like map tend to generate rvalues and makes the iterator "
        "not copy assignable.");
...
Cycle<Self>::Cycle(Selfx &&self)
    : self(self), ref_self(std::forward<Selfx>(self)) {
}
