...
public:
    ...
    /**
     * Type alias to the cloned type after removing const and reference
     * specifiers.
     */
...
template <class Self>
template <class Selfx>
Cloned<Self>::Cloned(Selfx &&self) : self(std::forward<Selfx>(self)) {
}

template <class Self>
auto Cloned<Self>::next() -> Option<Item> {
    return self.next().map([](auto &&value) -> Item { return value; });
}
