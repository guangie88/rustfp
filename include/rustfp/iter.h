/**
 * Contains conversion from std containers/iterator to Rust style iterators so
 * that other operations can be used.
...
/**
 * Represents rustfp iterator type for generating immutable references to the
 * original values in the container.
...
public:
    ...
    static_assert(
        ...
        "Iter can only take iterable whose iterator dereferences to a "
        "reference type");
...
public:
    ...
    static_assert(
        std::is_lvalue_reference<Item>::value,
        "IterMut can only take iterable whose iterator dereferences to a "
        "reference type");
...
public:
    ...
    static_assert(
        !std::is_reference<MovedStdInputIterable>::value,
...
public:
     ...
     * the iterator types.
     */
    using Item = std::add_const_t<
        typename std::iterator_traits<typename std::common_type_t<
     ...
     */
    IterBeginEnd(
        StdBeginInputIterator &&begin_it, StdEndInputIterator &&end_it);
...
/**
 * Wraps a container type that has std-styled iterator, into rustfp iterator
...
/**
 * Wraps a container type that has std-styled iterator, into rustfp iterator
 * that generates mutable reference to the original values in the container.
 * @tparam StdInputIterable container type that is able to produce
 * std input iterator for begin and end.
 * @param input_iterable Reference of container
 * to accept for generate the items.
...
/**
 * Wraps a container type that has std-styled iterator, into rustfp iterator
 * that moves each of the original values in the container.
 * @tparam MovedStdInputIterable container type that is able to produce
 * std input iterator for begin and end.
 * @param input_iterable Moved container
 * to accept for generate the moved items.
...
template <
    ...
    class = std::enable_if_t<
        !std::is_lvalue_reference<MovedStdInputIterable>::value>>
...
auto next_impl(InputIterable &input_iterable, InputIterator &curr_it)
    -> Option<Item> {
...
Iter<StdInputIterable>::Iter(const StdInputIterable &input_iterable)
    : inputIterableRef(input_iterable), curr_it(std::cbegin(input_iterable)) {
}
...
IterMut<StdInputIterable>::IterMut(StdInputIterable &input_iterable)
    : inputIterableRef(input_iterable), curr_it(std::begin(input_iterable)) {
}
...
template <class MovedStdInputIterable>
IntoIter<MovedStdInputIterable>::IntoIter(
    MovedStdInputIterable &&input_iterable)
...
template <class StdBeginInputIterator, class StdEndInputIterator>
auto IterBeginEnd<StdBeginInputIterator, StdEndInputIterator>::next()
    -> Option<Item> {
    if (curr_it != end_it) {
        const auto prev_it = curr_it;
        ++curr_it;
...
auto iter(StdInputIterable &&input_iterable)
    ...
    static_assert(
        std::is_lvalue_reference<StdInputIterable>::value,
...
auto iter_mut(StdInputIterable &input_iterable)
    ...
    static_assert(
        !std::is_const<StdInputIterable>::value,
        "rustfp::iter_mut() must be invoked on non-const lvalue reference "
        "only");
