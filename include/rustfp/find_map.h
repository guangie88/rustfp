...
public:
    ...
    template <class Self>
    auto operator()(Self &&self) && -> std::result_of_t<F(typename Self::Item)>;
...
template <class F>
template <class Fx>
FindMapOp<F>::FindMapOp(Fx &&f) : f(std::forward<Fx>(f)) {
}

template <class F>
template <class Self>
auto FindMapOp<F>::
operator()(Self &&self) && -> std::result_of_t<F(typename Self::Item)> {
