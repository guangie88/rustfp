...
public:
    ...
    /**
     * Takes in any other value other than Once type. This forwarding
     * constructor contains a second special template argument to prevent
     * conflict with the default copy/move constructor.
    ...
    template <
        ...
        class = std::enable_if_t<!std::is_same<
            Once<T>,
            std::remove_const_t<std::remove_reference_t<Tx>>>::value>>
...
Once<T>::Once(Tx &&value) : value(Some(std::forward<Tx>(value))) {
}
